...
if (selectedProduct) {
  ...
  if (
    ...
    function addToCart() {
      ...
      let getAllusersData = JSON.parse(
        localStorage.getItem("AllRegisteredUsers")
      );
      let loggedInuserData = getAllusersData.find(
        (obj) => obj.email == getUserInfo.email
      ); //a obj
      // console.log("pdpage loggedInuserData-", loggedInuserData);
      // check if the selected product is in the user's cart or not
      let isProductAlreadyAdded = loggedInuserData["CartProducts"].find(
        (obj) => obj.id == selectedProduct.id
      ); // give a obj or undefined
      ...
      if (isProductAlreadyAdded) {
        ...
        localStorage.setItem(
          "AllRegisteredUsers",
          JSON.stringify(getAllusersData)
        );
